...
std::vector<std::string> knight_moves(std::string knight){
    ...
    std::vector<std::string> moves; // the solution - the possible moves
    int x, y; // location of the knight
    x = knight[0] - 'a' + 1; // letter part of the location - column
    y = knight[1] - '0';     // number part of the location - row
    const int delta1[2] = {-1, 1}; // the shift we are doing
